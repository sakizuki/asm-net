...
def get_arguments():
    ...
    parser.add_argument('--proot', type=str, 
                        default='../params',
    ...
    parser.add_argument('--ddim', type=int, 
                        default = 3,
                        help='dimension of deformation parameter')
    parser.add_argument('--use_icp', 
                        action='store_true',
                        help='flag to use ICP algorithm for post processing')
    parser.add_argument('--use_gt_mask', 
                        action='store_true',
                        help='flag to use identical mask of object')
    parser.add_argument('--use_mean_shape', 
                        action='store_true',
                        help='flag to use mean shape')    
    parser.add_argument('--out_dir', type=str,
                        default='None',
                        help='output dir name')    
    parser.add_argument('--vis',
                        action='store_true',
                        help='make result image')    
...
if use_mean_shape:
    ...
    if use_icp:
        save_dir = os.path.join('release_mean_out', "{}_wICP_{}".format('real_test', now))
else:
    save_dir = os.path.join('release_proposed_out', "{}_woICP_{}".format('real_test', now))
    if use_icp:
        save_dir = os.path.join('release_proposed_out', "{}_wICP_{}".format('real_test', now))
...
models = cr6d_utils.load_models_release( osp.join(param_root,'weights'), synset_names, asm_dim, n_points, device )
...
for i, image_id in enumerate(image_ids):
    ...
    mask_path = osp.join(scene_mask_root,"results_{}_{}_{}.npz".format(ipp[-3],ipp[-2],ipp[-1]))
    ...
    scale_coeffs, model_cids, gt_pcds = cr6d_utils.get_model_scale( image_path, model_root )
    ...
    for m, im_m in enumerate(im_masks):
        ...
        pcd_obj = cr6d_utils.get_pcd_from_rgbd( im_c.copy(), im_md.copy(), camera_intrinsic )
        ...
        with torch.no_grad():
            ...
            if use_mean_shape == True:
                ...
                pcd_pred.scale( pred_scaling_param, (0.,0.,0.) )

            # convert real scale
            pcd_pred_vis = copy.deepcopy(pcd_pred)  
            pcd_pred_vis.scale( scale, (0.,0.,0.) )
            ...
            if use_icp:
                ...
                reg_result = o3.pipelines.registration.registration_icp( pcd_pred_visible, pcd_in,
                                                            max_correspondence_distance = 0.02)
                pcd_pred_vis = copy.deepcopy(pcd_pred_vis_ds).transform(reg_result.transformation)
    ...
    pcd_scene = None
    if vis == True:
        pcd_scene = cr6d_utils.get_pcd_from_rgbd( im_c.copy(), im_d.copy(), camera_intrinsic )
        im_objs = np.zeros(im_c.shape).astype(np.float)
        for pcd in pcd_pred_vises:
            img = mapping.Cloud2Image( pcd, True ).astype(np.float)
            im_objs += img
        im_imposed = ((im_objs.astype(np.float) + im_c.astype(np.float))/2).astype(np.uint8)
